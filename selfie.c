...
// +----+-----------------+
// | 32 | pid				| process id of this context
// | 33 | parent process	| context of parent process
// | 34 | process state	| current execution state of processss
...
uint64_t STATUS_RUNNING = 1;
uint64_t STATUS_READY = 2;
uint64_t STATUS_BLOCKED = 3;
uint64_t STATUS_FREED = 4;
uint64_t STATUS_ZOMBIE = 5;
...
uint64_t pid(uint64_t* context) { return (uint64_t) (context + 32); }
uint64_t parent_process(uint64_t* context) { return (uint64_t) (context + 33); }
uint64_t status(uint64_t* context) { return (uint64_t) (context + 34); }
...
uint64_t get_pid(uint64_t* context) { return 	*(context + 32); }
uint64_t* get_parent_process(uint64_t* context) { return (uint64_t*) *(context + 33); }
uint64_t get_status(uint64_t* context) { return *(context + 34); }
...
void set_pid(uint64_t* context, uint64_t pid)	{ *(context + 32) = pid; }
void set_parent_process(uint64_t* context, uint64_t* parent_process) { *(context + 33) = (uint64_t) parent_process; }
void set_status(uint64_t* context, uint64_t status) { *(context + 34) = status; }
...
uint64_t selfie_run(uint64_t machine, uint64_t nproc);
...
void implement_fork(uint64_t* context) {
	...
	/* Copy context entries from parent */
	set_pc(child, get_pc(context) + INSTRUCTIONSIZE);		// Copy pc
	set_name(child, string_copy(itoa(get_pid(child), integer_buffer, 10, 0, 0)));		// Copy name

	/* Copy registers */
	i = 0;
	while (i < NUMBEROFREGISTERS) {
		*(get_regs(child) + i) = *(get_regs(context) + i);		// Copy registers
		i = i + 1;
	}
	...
	set_ec_syscall(child, get_ec_syscall(context));
	set_ec_page_fault(child, get_ec_page_fault(context));
...
void free_context(uint64_t* context) {
  set_status(context, STATUS_FREED);
...
void block_context(uint64_t* context) {
	...
	/* Since we are moving to the blocked list, used_contexts shouldn't point
		to this context */
	if (used_contexts == context) used_contexts = get_next_context(context);
	...
	set_status(context, STATUS_BLOCKED);
